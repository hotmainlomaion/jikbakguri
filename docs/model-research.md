...
## 부록: 로컬 챗 모델 실측 (2026-07-04, EVA 대체 벤치)

EVA-Qwen 14B의 간헐적 언어 누출(중국어/독일어/러시아어)을 개선하려 5개 후보를 동일 한국어 NSFW RP 프롬프트 3종으로 실측. Rin 페르소나(발랄 반말·ㅎㅎ) 기준, 4축(한국어 자연성·코히런스·반말 일관·NSFW 순응)을 병렬 심사로 채점.

| 순위 | 모델 | 자연성 | 반말 | NSFW | 속도 | 비고 |
|---|---|---|---|---|---|---|
| **1** | **mistral-nemo (12B)** | 3 | 4 | 3 | ~10s | 반말·NSFW·속도 균형 최고, 코히런스만 약점 → **채택** |
| 2 | aya-expanse-abliterated (8B) | 4 | 2 | 2 | ~23s | 문장 완성도 최고나 존댓말 혼입·NSFW 회피 |
| 3 | gemma-2-9b-it-abliterated | 2 | 4 | 3 | ~12s | 반말·NSFW 좋으나 번역투 비문 |
| 4 | eva-qwen2.5-14b | 3 | 3 | 3 | ~34s | 이물질 토큰 누출 + 최느림 |
| 5 | dolphin3 (8B) | 2 | 3 | 1 | ~13s | NSFW 회피(관계를 '친구'로 축소) |

**핵심 발견**
- **언어 누출은 프롬프트 의존적**: 시스템 프롬프트에 "한국어로만(no Chinese/Russian)"을 명시하면 통제 벤치에서 5개 모두 누출 0. 앱 페르소나 프롬프트의 언어 고정 문구 유지가 1차 방어.
- **채택: `mistral-nemo`** (`ATLAS_LLM_MODEL`). 인앱 실측에서 도발적 유도에도 거부 없이 자연스러운 반말로 응하며(앱의 NSFW-enablement 프롬프트가 벤치의 소극성을 상쇄) 누출 0, ~10s로 최속.
- **트레이드오프**: mistral-nemo는 base(비-abliterated)라 극단적 하드코어에서 드물게 거부 가능 → 그 경우 `gemma-2-9b-it-abliterated`(uncensored 보장, 반말·NSFW 양호, 한국어 정제도만 낮음)로 대체. 상용 단계에선 호스티드 uncensored API 또는 자체 GPU의 더 큰 RP 파인튜닝으로 승급(§ 본문).
