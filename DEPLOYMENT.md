...
- 이미지 백엔드 계약: `POST { prompt, style:'photoreal'|'anime', seed, steps } → { b64 }` 또는 `{ url }`.
  캐릭터의 `image_style`/`image_seed`(0008)가 이 분기와 일관성을 만든다. 상용 백엔드는 이 계약만
  맞추면 되고, `style`로 photoreal(FLUX/실사 체크포인트)과 anime(Pony/Illustrious/Animagine)로 분기.
