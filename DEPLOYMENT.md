...
- **가장 빠른 승급(코드 0)**: 자체 GPU에 로컬과 동일한 이미지 서버 계약을 올리고 URL만 교체. `image_style`/`image_seed`(0008) 일관성 그대로. → 배포용 컨테이너·가이드: [`deploy/image-server/`](deploy/image-server/README.md) (CUDA diffusers, 웜 모델, 동일 계약). `MOCK=1`로 GPU 없이 계약·앱 통합 검증 가능(실측 완료).
...
- [ ] **성인 인증 실연동(필수)**: `/api/verify`는 실제 인증기관 연동 전엔 production에서 **501로 봉쇄**됨(P5 #2). `AGE_VERIFY_PROVIDER`/`AGE_VERIFY_API_KEY` 설정 + `verifyWithProvider()` 구현이 배포 선행조건(7-A: 자가통과 스텁 금지).
- [ ] **RLS 자가변경 차단(필수)**: `users.is_adult_verified`/`status`는 클라(anon/authenticated) UPDATE 권한 회수 확인(0014, P5 #1). 자가 성인인증·밴해제 불가.
- [ ] **입력 모더레이션**: 채팅·이미지 프롬프트가 AI 호출 **전** heuristicScan(미성년/불법, 영·한 완곡어/수사나이/정규화우회 포함) 통과. 위반 시 차단+`moderation_logs`.
- [ ] **텍스트 분류기 실연동(필수)**: `MODERATION_TEXT_URL`/`KEY` **미설정 시 production은 fail-closed**로 텍스트 AI 경로 차단(P5 #3). heuristic은 백스톱일 뿐 — 실분류기 설정이 배포 선행조건.
